...
function header(){return `<header class="topbar"><div class="topbar-inner">${logo()}<div class="spacer"></div>${state.profile?`<div class="mobile-header-actions"><button class="btn btn-secondary mobile-language-button" data-action="toggle-language" data-language-button="1" title="${state.language==='zh-CN'?'切换到葡萄牙语':'Traduzir todo o site para mandarim'}">${state.language==='zh-CN'?'PT':'中文'}</button><button class="btn btn-secondary mobile-logout-button" data-action="logout">Sair</button></div><div class="top-actions"><span class="desktop-only" style="font-size:11px;opacity:.8">${esc(state.profile.email||'')}</span>${state.profile.role==='admin'?`<button class="btn btn-secondary" data-action="route" data-route="#/admin">Administração</button>`:''}<button class="btn btn-secondary" data-action="route" data-route="#/catalog">Catálogo</button><button class="btn btn-secondary account-button" data-action="route" data-route="#/account">Minha conta</button><button class="btn btn-primary" data-action="open-cart">Carrinho (${cartCount()})</button><button class="btn btn-secondary desktop-language-button" data-action="toggle-language" data-language-button="1" title="${state.language==='zh-CN'?'切换到葡萄牙语':'Traduzir todo o site para mandarim'}">${state.language==='zh-CN'?'PT':'中文'}</button><button class="btn btn-secondary desktop-logout-button" data-action="logout">Sair</button></div>`:''}</div></header>`}
