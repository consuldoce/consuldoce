...
function setLanguage(lang){state.language=lang==='zh-CN'?'zh-CN':'pt';localStorage.setItem('consuldoce_language',state.language);let tries=0;const run=()=>{if(applyGoogleLanguage()||tries++>25){render().catch(()=>{});return}setTimeout(run,180)};run()}
