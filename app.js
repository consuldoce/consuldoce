...
function filteredAdminOrders(){
  ...
  const name=o=>String(o.profiles?.full_name||o.client_name_snapshot||'').toLocaleLowerCase('pt-PT');
...
function renderAdminOrderRows(list){
  const body=document.getElementById('adminOrdersBody');if(!body)return;
  const summary=document.getElementById('adminOrdersSummary');if(summary)summary.textContent=`${list.length} de ${(window.__adminOrders||[]).length} encomenda${(window.__adminOrders||[]).length===1?'':'s'}`;
...
async function adminClients(c){
  ...
  const countries=[...new Set(window.__adminClients.map(p=>p.country).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt'));
  ...
  <div class="toolbar admin-list-toolbar" style="margin-top:14px"><input id="adminClientSearch" class="field search" placeholder="Pesquisar por nome, NIF, email, telefone ou morada..." data-action="filter-admin-clients"><select id="adminClientCountry" class="field category" data-action="filter-admin-clients"><option value="">Todos os países</option>${countries.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('')}</select><select id="adminClientSort" class="field sort" data-action="filter-admin-clients"><option value="name_asc">Nome A–Z</option><option value="name_desc">Nome Z–A</option><option value="nif_asc">NIF A–Z</option><option value="nif_desc">NIF Z–A</option><option value="email_asc">Email A–Z</option><option value="email_desc">Email Z–A</option><option value="date_desc">Registo mais recente</option><option value="date_asc">Registo mais antigo</option></select></div>
  <div id="adminClientsSummary" class="hint" style="margin:-7px 0 12px"></div>
  ...
  renderAdminClientRows(filteredAdminClients());
}
function adminClientSearchText(p){return [p.full_name,p.nif,p.email,p.phone_country_code,p.phone_number,p.address,p.address_line1,p.address_line2,p.postal_code,p.postal_locality,p.country].filter(v=>v!==null&&v!==undefined).join(' ').toLocaleLowerCase('pt-PT')}
function filteredAdminClients(){
  const all=window.__adminClients||[];
  const q=String(document.getElementById('adminClientSearch')?.value||'').trim().toLocaleLowerCase('pt-PT');
  const country=String(document.getElementById('adminClientCountry')?.value||'');
  const sort=String(document.getElementById('adminClientSort')?.value||'name_asc');
  const list=all.filter(p=>(!q||adminClientSearchText(p).includes(q))&&(!country||p.country===country));
  const val=(p,k)=>String(p[k]??'').toLocaleLowerCase('pt-PT');
  list.sort((a,b)=>{if(sort==='name_desc')return val(b,'full_name').localeCompare(val(a,'full_name'),'pt');if(sort==='nif_asc')return val(a,'nif').localeCompare(val(b,'nif'),'pt');if(sort==='nif_desc')return val(b,'nif').localeCompare(val(a,'nif'),'pt');if(sort==='email_asc')return val(a,'email').localeCompare(val(b,'email'),'pt');if(sort==='email_desc')return val(b,'email').localeCompare(val(a,'email'),'pt');if(sort==='date_asc')return new Date(a.created_at)-new Date(b.created_at);if(sort==='date_desc')return new Date(b.created_at)-new Date(a.created_at);return val(a,'full_name').localeCompare(val(b,'full_name'),'pt')});
  return list;
}
...
function filterAdminClients(){renderAdminClientRows(filteredAdminClients())}
...
document.addEventListener('input',(event)=>{
  ...
  try{
    ...
    else if(a==='filter-admin-orders') filterAdminOrders();
