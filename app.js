...
const state = { session:null, profile:null, addresses:[], products:[], categories:[], cart:loadCart(), route:location.hash||'#/catalog', authMode:'login', adminTab:'orders', importMode:'articles', loading:false, recoveryFlow:false, selectedClientId:null };
...
let countryListSeq=0;function countryOptions(selected='Portugal'){const names=[...new Set(ADDRESS_COUNTRIES.map(([,name])=>name).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt'));const listId=`countryList-${++countryListSeq}`;return `<input class=\"field country-input\" name=\"country\" value=\"${esc(selected||'Portugal')}\" list=\"${listId}\" autocomplete=\"country-name\" required placeholder=\"Pesquisar país…\"><datalist id=\"${listId}\">${names.map(name=>`<option value=\"${esc(name)}\"></option>`).join('')}</datalist>`}
...
function header(){return `<header class="topbar"><div class="topbar-inner">${logo()}<div class="spacer"></div>${state.profile?`<div class="top-actions"><span class="desktop-only" style="font-size:11px;opacity:.8">${esc(state.profile.email||'')}</span>${state.profile.role==='admin'?`<button class="btn btn-secondary" data-action="route" data-route="#/admin">Administração</button>`:''}<button class="btn btn-secondary" data-action="route" data-route="#/catalog">Catálogo</button><button class="btn btn-secondary account-button" data-action="route" data-route="#/account">Minha conta</button><button class="btn btn-primary" data-action="open-cart">Carrinho (${cartCount()})</button><button class="btn btn-secondary" data-action="logout">Sair</button></div>`:''}</div></header>`}

function auth(){const forgot=state.authMode==='forgot';const recovery=state.authMode==='recovery';if(state.authMode==='registered')return `<div class=\"auth-shell\"><div class=\"auth-card\"><div class=\"auth-logo\">${logo()}</div><div class=\"eyebrow\">Registo concluído</div><h1 class=\"h1\">Conta registada com sucesso</h1><p class=\"lead\" style=\"margin-bottom:18px\">A sua conta foi criada. Para concluir o registo, consulte o email enviado para o endereço indicado e clique no link de confirmação.</p><div class=\"notice success\"><strong>Confirme o seu email</strong><br>Depois da confirmação poderá voltar a esta página e iniciar sessão.</div><button class=\"btn btn-primary\" style=\"width:100%;margin-top:18px\" data-action=\"registered-back-login\">Voltar ao login</button><p class=\"footer-note\">Se não encontrar o email, verifique também a pasta de spam.</p></div></div>`;if(forgot)return `<div class="auth-shell"><div class="auth-card"><div class="auth-logo">${logo()}</div><div class="eyebrow">Recuperação de acesso</div><h1 class="h1">Esqueci-me da palavra-passe</h1><p class="lead" style="margin-bottom:20px">Indique o email da conta e enviaremos instruções para definir uma nova palavra-passe.</p><form id="authForm" class="stack"><div class="form-group"><label class="label">Email</label><input class="field" name="email" type="email" required autocomplete="email"></div><button class="btn btn-primary" type="submit">Enviar instruções</button></form><div id="authMsg" style="margin-top:12px"></div><p class="footer-note"><button class="link-button" data-action="auth-login">Voltar ao login</button></p></div></div>`;if(recovery)return `<div class="auth-shell"><div class="auth-card"><div class="auth-logo">${logo()}</div><div class="eyebrow">Recuperação de acesso</div><h1 class="h1">Definir nova palavra-passe</h1><p class="lead" style="margin-bottom:20px">Escolha uma nova palavra-passe com pelo menos 8 caracteres.</p><form id="authForm" class="stack"><div class="form-group"><label class="label">Nova palavra-passe</label><input class="field" name="password" type="password" required minlength="8" autocomplete="new-password"></div><div class="form-group"><label class="label">Confirmar palavra-passe</label><input class="field" name="password_confirm" type="password" required minlength="8" autocomplete="new-password"></div><button class="btn btn-primary" type="submit">Guardar nova palavra-passe</button></form><div id="authMsg" style="margin-top:12px"></div><p class="footer-note"><button class="link-button" data-action="auth-login">Voltar ao login</button></p></div></div>`;return `<div class="auth-shell"><div class="auth-card"><div class="auth-logo">${logo()}</div><div class="eyebrow">Acesso seguro</div><h1 class="h1">${state.authMode==='login'?'Entrar no catálogo':'Criar acesso de cliente'}</h1><p class="lead" style="margin-bottom:20px">${state.authMode==='login'?'Encomende a mercadoria disponível sem visualizar preços.':'Registe os dados da sua empresa para começar.'}</p><div class="tabs"><button class="tab ${state.authMode==='login'?'active':''}" data-action="auth-login-render">Entrar</button><button class="tab ${state.authMode==='signup'?'active':''}" data-action="auth-signup-render">Registar</button></div><form id="authForm" class="stack">${state.authMode==='signup'?`<div class="form-grid"><div class="form-group full"><label class="label">Nome / empresa</label><input class="field" name="full_name" required maxlength="150" autocomplete="organization"></div><div class="form-group"><label class="label">NIF</label><input class="field" name="nif" required maxlength="20" autocomplete="off"></div><div class="form-group full"><label class="label">Telemóvel</label><div class="phone-field"><select class="field phone-prefix" name="phone_country_code" aria-label="País e indicativo">${phoneCountryOptions('+351')}</select><input class="field phone-number" name="phone_number" type="tel" required inputmode="tel" autocomplete="tel" maxlength="30" placeholder="912 345 678"></div></div><div class="form-group full"><label class="label">Endereço principal</label><input class="field" name="address_line1" required maxlength="250" autocomplete="street-address"></div><div class="form-group full"><label class="label">Andar, lote, fração, porta, etc. <span class="hint">(opcional)</span></label><input class="field" name="address_line2" maxlength="150" autocomplete="address-line2"></div><div class="form-group"><label class="label">Código postal</label><input class="field" name="postal_code" required maxlength="20" autocomplete="postal-code"></div><div class="form-group"><label class="label">Localidade postal</label><input class="field" name="postal_locality" required maxlength="120" autocomplete="address-level2"></div><div class="form-group full"><label class="label">País</label>${countryOptions('Portugal')}</div></div>`:''}<div class="form-group"><label class="label">Email</label><input class="field" name="email" type="email" required autocomplete="email"></div><div class="form-group"><label class="label">Palavra-passe</label><input class="field" name="password" type="password" required minlength="8" autocomplete="${state.authMode==='login'?'current-password':'new-password'}"></div><button class="btn btn-primary" type="submit">${state.authMode==='login'?'Entrar':'Criar conta'}</button></form>${state.authMode==='login'?'<button class="link-button" style="margin-top:14px" data-action="forgot-password">Esqueci-me da palavra-passe</button>':''}<div id="authMsg" style="margin-top:12px"></div><p class="footer-note">Catálogo privado B2B · Consuldoce</p></div></div>`}
...
function forcePassword(){return `<div class="auth-shell"><div class="auth-card"><div class="auth-logo">${logo()}</div><div class="eyebrow">Segurança da conta</div><h1 class="h1">Defina uma nova palavra-passe</h1><p class="lead" style="margin-bottom:20px">Por motivos de segurança, a administração pediu-lhe para alterar a palavra-passe antes de continuar.</p><form id="forcePasswordForm" class="stack"><div class="form-group"><label class="label">Nova palavra-passe</label><input class="field" name="password" type="password" required minlength="8" autocomplete="new-password"></div><div class="form-group"><label class="label">Confirmar nova palavra-passe</label><input class="field" name="password_confirm" type="password" required minlength="8" autocomplete="new-password"></div><button class="btn btn-primary" type="submit">Guardar nova palavra-passe</button></form><div id="forcePasswordMsg" style="margin-top:12px"></div></div></div>`}
...
async function sendAccountPasswordReset(){try{const email=String(state.profile?.email||state.session?.user?.email||'').trim().toLowerCase();if(!email)throw new Error('Não foi possível identificar o email da conta.');const redirectTo=`${location.origin}/recovery`;const {error}=await sb.auth.resetPasswordForEmail(email,{redirectTo});if(error)throw error;document.getElementById('accountPasswordMsg').innerHTML='<div class="notice success">Foi enviado um email de redefinição para o endereço registado. Verifique também a pasta de spam.</div>'}catch(err){document.getElementById('accountPasswordMsg').innerHTML=`<div class="notice danger">${esc(err.message||'Não foi possível enviar o email de redefinição.')}</div>`}}
...
function catalog(){const cats=state.categories.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('');return `${header()}<main class="page"><div class="hero"><div><div class="eyebrow">Catálogo vivo</div><h1 class="h1">Produtos para encomenda</h1><p class="lead">Escolha as quantidades e envie a sua encomenda à Consuldoce. Os preços não são apresentados.</p></div><button class="btn btn-light" data-action="open-orders">As minhas encomendas</button></div><div class="toolbar"><input id="search" class="field search" placeholder="Pesquisar por produto, referência ou código de barras..." data-action="filter-products"><select id="cat" class="field category" data-action="filter-products"><option value="">Todas as famílias</option>${cats}</select><label class="stock-filter"><input id="onlyInStock" type="checkbox" checked data-action="filter-products"> Apenas em stock</label><select id="sort" class="field sort" data-action="filter-products"><option value="name-asc">Nome A–Z</option><option value="name-desc">Nome Z–A</option><option value="category-asc">Categoria A–Z</option><option value="category-desc">Categoria Z–A</option><option value="sku-asc">ID do produto A–Z</option><option value="sku-desc">ID do produto Z–A</option></select></div><div id="catalogGrid" class="catalog-grid"></div><div class="footer-note">Consuldoce · Catálogo B2B privado</div></main><div id="cartMount"></div><div id="ordersMount"></div>`}
...
function productCard(p){const qty=Number(state.cart[p.id]||0),available=productInStock(p);return `<article class="card"><div class="product-image">${p.image_url?`<img src="${esc(p.image_url)}" alt="${esc(p.name)}" loading="lazy">`:'<div class="placeholder">Imagem do produto<br>por adicionar</div>'}</div><div class="card-body"><span class="tag">${esc(p.category||'Produto')}</span><div class="product-name">${esc(p.name)}</div><div class="sku">Ref. ${esc(p.sku||'—')}${p.barcode?` · ${esc(p.barcode)}`:''}</div><div class="stock ${available?'':'off'}">${available?'Em stock':'Fora de stock'}</div><div class="qty-row"><div class="qty"><button ${available?'':'disabled'} data-action="qty-minus" data-id="${esc(p.id)}">−</button><input aria-label="Quantidade" value="${qty}" inputmode="numeric" ${available?'':'disabled'} data-action="set-qty" data-id="${esc(p.id)}"><button ${available?'':'disabled'} data-action="qty-plus" data-id="${esc(p.id)}">+</button></div><button class="btn btn-primary" ${available?'':'disabled'} data-action="qty-add" data-id="${esc(p.id)}" data-value="${qty+1}">Adicionar</button></div></div></article>`}
...
async function adminOrders(c){const {data,error}=await sb.from('orders').select('id,status,notes,created_at,client_name_snapshot,client_email_snapshot,client_nif_snapshot,delivery_address_label,delivery_address_line1,delivery_address_line2,delivery_postal_code,delivery_postal_locality,delivery_country,profiles(full_name,email,nif,address),order_items(product_name,sku,quantity)').order('created_at',{ascending:false});if(error){c.innerHTML=`<div class="panel danger">${esc(error.message)}</div>`;return}const counts={new:0,processing:0,completed:0,cancelled:0};(data||[]).forEach(o=>counts[o.status]=(counts[o.status]||0)+1);c.innerHTML=`<div class="stats"><div class="stat"><div class="hint">Novas</div><div class="stat-num">${counts.new}</div></div><div class="stat"><div class="hint">Em preparação</div><div class="stat-num">${counts.processing}</div></div><div class="stat"><div class="hint">Concluídas</div><div class="stat-num">${counts.completed}</div></div></div><div class="panel"><h2 class="panel-title">Encomendas</h2><p class="hint">Ações de estado e reenvio do email.</p><div class="table-wrap"><table class="table"><thead><tr><th>Data</th><th>Cliente</th><th>Produtos</th><th>Estado</th><th>Ações</th></tr></thead><tbody>${(data||[]).map(o=>`<tr><td>${new Date(o.created_at).toLocaleString('pt-PT')}</td><td><strong>${esc(o.profiles?.full_name||o.client_name_snapshot||'—')}</strong><br>${esc(o.profiles?.nif||o.client_nif_snapshot||'')}<br><span class="hint">${esc(o.profiles?.email||o.client_email_snapshot||'')}</span><br><span class="hint">Entrega: ${esc([o.delivery_address_line1,o.delivery_address_line2,o.delivery_postal_code,o.delivery_postal_locality,o.delivery_country].filter(Boolean).join(', ')||o.profiles?.address||'—')}</span></td><td>${(o.order_items||[]).map(i=>`${esc(i.product_name)} × ${i.quantity}`).join('<br>')}</td><td><select class="field" style="width:auto;padding:7px" data-action="order-status" data-id="${esc(o.id)}">${['new','processing','completed','cancelled'].map(s=>`<option ${o.status===s?'selected':''}>${s}</option>`).join('')}</select></td><td><button class="btn btn-light btn-small" data-action="resend-email" data-id="${esc(o.id)}">Reenviar email</button></td></tr>`).join('')}</tbody></table></div></div>`}
...
async function adminClients(c){
  const {data,error}=await sb.from('profiles').select('id,email,full_name,nif,phone_country_code,phone_number,address,address_line1,address_line2,postal_code,postal_locality,country,role,created_at,must_change_password').order('created_at',{ascending:false});
...
async function render(){if(!state.session){app.innerHTML=auth();document.getElementById('authForm')?.addEventListener('submit',doAuth);return}if(state.route==='#/admin' && state.profile?.role!=='admin'){state.route='#/catalog'}if(state.route==='#/force-password'){app.innerHTML=forcePassword();document.getElementById('forcePasswordForm')?.addEventListener('submit',changeForcedPassword);return}if(state.route==='#/admin'){app.innerHTML=admin();await renderAdmin()}else if(state.route==='#/account'){app.innerHTML=account();document.getElementById('accountProfileForm')?.addEventListener('submit',saveAccountProfile);document.getElementById('accountPasswordForm')?.addEventListener('submit',changeAccountPassword)}else{await loadProducts();app.innerHTML=catalog();filterProducts()}}
...
document.addEventListener('click',(event)=>{
  ...
  try{
    ...
    else if(a==='account-password-reset-email') sendAccountPasswordReset();
