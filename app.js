...
const state = { session:null, profile:null, addresses:[], products:[], categories:[], cart:loadCart(), route:location.hash||'#/catalog', authMode:'login', adminTab:'orders', importMode:'articles', loading:false, recoveryFlow:false, selectedClientId:null, language:localStorage.getItem('consuldoce_language')||'pt' };
...
async function render(){if(!state.session){app.innerHTML=auth();document.getElementById('authForm')?.addEventListener('submit',doAuth);setTimeout(applyGoogleLanguage,120);return}if(state.route==='#/admin' && state.profile?.role!=='admin'){state.route='#/catalog'}if(state.route==='#/force-password'){app.innerHTML=forcePassword();document.getElementById('forcePasswordForm')?.addEventListener('submit',changeForcedPassword);setTimeout(applyGoogleLanguage,120);return}if(state.route==='#/admin'){app.innerHTML=admin();await renderAdmin()}else if(state.route==='#/account'){app.innerHTML=account();document.getElementById('accountProfileForm')?.addEventListener('submit',saveAccountProfile);document.getElementById('accountPasswordForm')?.addEventListener('submit',changeAccountPassword)}else{await loadProducts();app.innerHTML=catalog();filterProducts()}setTimeout(applyGoogleLanguage,120)}
