...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: customerAddresses } = await admin.from('customer_addresses').select('label,address_line1,address_line2,postal_code,postal_locality,country,is_default').eq('client_id', order.client_id).order('is_default',{ascending:false}).order('created_at',{ascending:true})
    ...
    const registeredAddress = [client?.address_line1, client?.address_line2, client?.postal_code, client?.postal_locality, client?.country].filter((v: any) => String(v ?? '').trim()).join(', ') || client?.address || ''
    const deliveryLabel = order?.delivery_address_label ? ` (${escapeHtml(order.delivery_address_label)})` : ''
    const savedAddressesHtml = (customerAddresses ?? []).map((a: any) => `<li><strong>${escapeHtml(a.label || 'Morada')}</strong>${a.is_default ? ' — predefinida' : ''}: ${escapeHtml([a.address_line1,a.address_line2,a.postal_code,a.postal_locality,a.country].filter((v:any)=>String(v??'').trim()).join(', '))}</li>`).join('')
    ...
    const html = `<!doctype html><html><body style="font-family:Arial,sans-serif;color:#201627"><h2>Nova encomenda Consuldoce</h2><p><strong>Encomenda:</strong> ${order.id}</p><p><strong>Data:</strong> ${new Date(order.created_at).toLocaleString('pt-PT')}</p><h3>Dados do cliente</h3><p><strong>Nome / empresa:</strong> ${escapeHtml(client?.full_name ?? '')}<br><strong>NIF:</strong> ${escapeHtml(client?.nif ?? '')}<br><strong>Email:</strong> ${escapeHtml(client?.email ?? '')}<br><strong>Telemóvel:</strong> ${escapeHtml(phone)}<br><strong>Morada atualmente registada:</strong> ${escapeHtml(registeredAddress || '—')}</p><h3>Morada de entrega${deliveryLabel}</h3><p>${escapeHtml(structuredAddress || '—')}</p>${savedAddressesHtml ? `<h3>Moradas no cadastro</h3><ul>${savedAddressesHtml}</ul>` : ''}${stockWarning}<table cellpadding="0" cellspacing="0" style="border-collapse:collapse;width:100%;max-width:800px"><thead><tr><th style="text-align:left;padding:8px;background:#f5eef6">Ref.</th><th style="text-align:left;padding:8px;background:#f5eef6">Produto</th><th style="text-align:right;padding:8px;background:#f5eef6">Qtd.</th></tr></thead><tbody>${rows}</tbody></table>${order.notes ? `<p><strong>Observações:</strong><br>${escapeHtml(order.notes)}</p>` : ''}</body></html>`
