...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const structuredAddress = [client?.address_line1, client?.address_line2, client?.postal_code, client?.postal_locality, client?.country].filter((v: any) => String(v ?? '').trim()).join(', ') || client?.address || ''
